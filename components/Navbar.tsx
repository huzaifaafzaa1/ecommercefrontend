...
const Navbar = () => {
  ...
  const menuItems = [
    {
      icon: <img src="logo.png" alt="Logo" className="h-6 max-w-6" />,
      text: "Logo",
      href: null,
    },
    ...
    {
      icon: <RiShoppingBag4Fill className="h-6 w-6" />,
      text: "Bag",
      href: "/bagItems",
    },
    {
      ...
      <nav className="navbar group sticky top-0 left-0 bg-white flex flex-col items-center justify-between w-16 h-[625px] rounded-xl mx-2 my-2 transition-all duration-300 ease-in-out hover:w-36">
        <ul className="flex flex-col gap-7 my-5 justify-center items-center">
          {menuItems.map((item, index) => (
            <li
              key={index}
              className="h-6 w-6 flex justify-center items-center group-hover:w-full"
            >
              {item.href ? (
                <Link
                  href={item.href}
        ...
        <div className="flex gap-2 justify-center items-center">
          ...
          <div className="hidden group-hover:block text-black font-cabin">
            Log Out
          </div>
        </div>
      </nav>
